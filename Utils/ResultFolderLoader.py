...
def _get_printable_recommender_name(recommender_name: str):
...
def _get_algorithm_file_name_list(algorithm_list,
    ...
    for algorithm in algorithm_list:
        ...
        if algorithm in [ItemKNNCFRecommender,
                         ...
                         ItemKNNCFBingewatcherBingeworthyRecommender,
                         UserKNNCFBingeworthyRecommender,
                         ItemKNNCFBingewatcherRecommender,
                         ItemKNNCFBingeworthyRecommender,
                         UserKNNCFBingewatcherBingeworthyRecommender,
                         UserKNNCFBingewatcherRecommender]:

            this_algorithm_data_list = _get_algorithm_similarity_and_feature_combinations(algorithm,
                                                                                          algorithm_row_label,
                                                                                          algorithm_file_name,
                                                                                          KNN_similarity_list,
                                                                                          [])
        ...
        # If KNN user content based or hybrid user based, expand similarity type and UCM names
        ...
        elif algorithm in [MatrixFactorization_BPR_Impression_Cython]:
            algorithm_data_to_print_list.append({
                "algorithm": algorithm,
            ...
            for impression_sampling in ["inside", "outside"]:
                algorithm_data_to_print_list.append({
                    "algorithm": algorithm,
                    "algorithm_row_label": f"{algorithm_row_label}{impression_sampling}",
                    "algorithm_file_name": f"{algorithm_file_name}_{impression_sampling}",
                })
...
def _get_algorithm_metadata_to_print_list(result_folder_path,
    ...
    for algorithm_file_dict in algorithm_file_name_list:
        ...
        if algorithm_file_name is not None:
            try:
                search_metadata = dataIO.load_data(f"{algorithm_file_name}_metadata")
...
def _time_string_builder(data_list, n_decimals=4):
    ...
    if len(data_list) > 0:
        ...
        if new_time_unit != "sec":
            ...
            result_row_string += " / " + _measure_unit_string(mean_new_unit, stddev_new_unit, new_time_unit,
...
def _format_hyperparameter_row_values(dataframe_row_series):
    ...
    for dataset_column in dataframe_row_series.index:
        ...
        if isinstance(hyperparameter_value, numbers.Real) and not isinstance(hyperparameter_value, numbers.Integral):
            ...
            if any(substring in hyperparameter_name for substring in
                   ["penalty", "rate", "reg", "l1", "lambda", "l2", "decay"]):
...
def _print_latex_hyperparameters_from_dataframe(hyperparameters_dataframe, hyperparameters_file):
    ...
    hyperparameters_dataframe = hyperparameters_dataframe[
        ~hyperparameters_dataframe.index.get_level_values("Algorithm").str.contains("algorithm_group")]
    ...
    header_wrong_code = " +& +" + "".join(
        ["& +{} ".format(dataset) for dataset in hyperparameters_dataframe.columns]) + "\\\\\\\\\n" + \
                        "Algorithm & Hyperparameter " + "& +" * n_datasets + "\\\\\\\\\n"

    header_correct_code = "Algorithm & Hyperparameter " + "".join(
        ["&\t {} ".format(dataset) for dataset in hyperparameters_dataframe.columns]) + "\\\\\\\\\n"
...
def _remove_missing_runs_for_algorithm(hyperparameters_dataframe):
    ...
    none_hyperpar = hyperparameters_dataframe[
        hyperparameters_dataframe.index.get_level_values("hyperparameter_name").isnull()]
    ...
    rows_to_drop = hyperparameters_dataframe.index.get_level_values("algorithm_row_label").isin(
        single_nan_to_remove_flag.index) & hyperparameters_dataframe.index.get_level_values(
...
def generate_latex_hyperparameters(result_folder_path,
    ...
    for experiment_subfolder in experiment_subfolder_list:
        ...
        result_loader = ResultFolderLoader("{}/{}/".format(result_folder_path, experiment_subfolder),
    ...
    if split_per_algorithm_type:
        ...
        for group_label, group_alg_list in algorithm_type_group.items():
            ...
            group_label_list = [_get_printable_recommender_name(recommender_class.RECOMMENDER_NAME) for
                                recommender_class in group_alg_list]
...
class ResultFolderLoader(object):
    ...
    _DEFAULT_BASE_ALGORITHM_LIST = [
        ...
        None,
        LightFMWrapper,
        LightFMWrapper_bingewatching,
        LightFMWrapper_bingeworthy,
        LightFMWrapper_bingewatching_bingeworthy,
        UserKNNCFBingewatcherBingeworthyRecommender,
        UserKNNCFBingewatcherRecommender,
        UserKNNCFBingeworthyRecommender,
        ItemKNNCFBingewatcherBingeworthyRecommender,
        ...
        ItemKNNCFBingeworthyRecommender,
        None,
        SLIMElasticNetRecommenderBingewatcherBingeworthy,
        SLIMElasticNetRecommenderBingewatcher,
        SLIMElasticNetRecommenderBingeworthy,
        # SLIMElasticNetRecommenderBingeworthy,
        # ItemKNNCFBingewatcherBingeworthyRecommender,
        # None,
        # ItemKNN_CFCBF_Hybrid_Recommender,
    ...
    def get_results_dataframe(self,
        ...
        algorithm_label_index = [row_dict["algorithm_row_label"]
                                 if row_dict is not None else "algorithm_group_{}".format(row_index)
    ...
    def generate_latex_results(self, output_file_path,
        ...
        if collapse_multicolumn_if_needed and (len(metrics_list) == 1):
            result_dataframe.columns = ['@'.join([self._METRIC_NAME_TO_LATEX_LABEL_DICT[col[1]], col[0]]).strip() for
                                        col in result_dataframe.columns.values]

        else:
            # Rename columns in such a way that they are nicely printable in latex
            result_dataframe.rename(
                columns={col_name: "@ {}".format(col_name) for col_name in result_dataframe.columns.levels[0]},
                level=0, inplace=True)

            result_dataframe.rename(columns={col_name: self._METRIC_NAME_TO_LATEX_LABEL_DICT[col_name] for col_name in
                                             result_dataframe.columns.levels[1]},
    ...
    def get_time_statistics_dataframe(self,
        ...
        algorithm_label_index = [
            row_dict["algorithm_row_label"] if row_dict is not None else "algorithm_group_{}".format(row_index) for
            row_index, row_dict in enumerate(self._metadata_list)]
        ...
        for row_index, row_dict in enumerate(self._metadata_list):
            ...
            if search_metadata is not None:
                ...
                if optimal_hyperparameters_index is None:
                    ...
                    optimal_hyperparameters_test_time = search_metadata["time_on_test_list"][
                        optimal_hyperparameters_index]
    ...
    def generate_latex_time_statistics(self, output_file_path,
        ...
        result_dataframe = self.get_time_statistics_dataframe(n_decimals=n_decimals,
                                                              n_evaluation_users=n_evaluation_users)
        output_file = open(output_file_path, "w")

        result_dataframe.rename(
            columns={"Recommendation Time": "\\begin{tabular}{@{}c@{}}Recommendation\\\\Time\\end{tabular}",
