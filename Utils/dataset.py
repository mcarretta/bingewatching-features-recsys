...
from SerialContentAnalysisFunctions import global_temporal_split, user_temporal_split, clean_dataset_CW_class, separate_dataset_into_chunks, _get_timestamp_differential_dataframe, clean_dataset
...
class ContentWiseImpressions(Dataset):
    ...
    by F.B. Pérez Maurera,   , Lorenzo Saule, Mario Scriminaci, and Paolo Cremonesi. If you use
    ...
    def read_dataset(self) -> None:
        ...
        self.interactions = self.interactions.compute()
    ...
    def _generate_urm_interactions(self, use_items: bool) -> None:
        ...
        self.interactions = clean_dataset_CW_class(self.interactions)
        logger.info(f"Performing user temporal split")

        train_data, validation_data, test_data = user_temporal_split(self.interactions)
        test_data = test_data[
            (test_data["user_id"].isin(train_data.user_id)) & (test_data["series_id"].isin(train_data.series_id))]
        validation_data = validation_data[(validation_data["user_id"].isin(train_data.user_id) & (
            validation_data["series_id"].isin(train_data.series_id)))]
        # num_users = self.metadata["num_users"]
        # num_items = self.metadata["num_items" if use_items else "num_series"]
        num_users, num_items = train_data.user_id.nunique(), train_data.series_id.nunique()
        num_interactions = self.interactions.shape[0]

        # Translation dictionary
        u_ids, s_ids = np.unique(train_data.user_id.values), np.unique(train_data.series_id.values)
        translation_user_id_index_urm = {int(u_ids[i]): i for i in range(len(u_ids))}
        translation_series_id_index_urm = {int(s_ids[i]): i for i in range(len(s_ids))}

        with open(os.path.join(self._local_dataset_dir, "translation_user_id_index_urm.json"), "w") as f:
            json.dump(obj=translation_user_id_index_urm, fp=f)
        with open(os.path.join(self._local_dataset_dir, "translation_series_id_index_urm.json"), "w") as f:
            json.dump(obj=translation_series_id_index_urm, fp=f)
        # URM_train building
        URM_train = np.zeros(shape=(num_users, num_items))
        train_chunks = separate_dataset_into_chunks(train_data)

        for chunk in tqdm(train_chunks):
            URM_train[
                translation_user_id_index_urm[chunk.user_id.values[0]], translation_series_id_index_urm[
                    chunk.series_id.values[0]]] = len(chunk)
        # URM_validation building
        URM_validation = np.zeros(shape=(num_users, num_items))
        validation_chunks = separate_dataset_into_chunks(validation_data)

        for chunk in tqdm(validation_chunks):
            URM_validation[
                translation_user_id_index_urm[chunk.user_id.values[0]], translation_series_id_index_urm[
                    chunk.series_id.values[0]]] = len(chunk)

        # URM_test building
        URM_test = np.zeros(shape=(num_users, num_items))
        test_chunks = separate_dataset_into_chunks(test_data)
        ...
        # # self.interactions = self.interactions.drop_duplicates(subset=[user_column, item_column])
        # # train_split, validation_split, test_split = user_temporal_split(self.interactions)
        # print("SHAPES")
        # print(self.interactions.shape, train_split.shape, validation_split.shape, test_split.shape)

        # train_data = dask.delayed(np.ones)(train_split.shape[0], dtype=np.int32)
        train_split = dask.delayed(csr_matrix)(URM_train,
                                               shape=(num_users, num_items),
                                               dtype=np.int32)

        # validation_data = dask.delayed(np.ones)(validation_split.shape[0], dtype=np.int32)
        validation_split = dask.delayed(csr_matrix)(URM_validation,
                                                    shape=(num_users, num_items),
                                                    dtype=np.int32)

        # test_data = dask.delayed(np.ones)(test_split.shape[0], dtype=np.int32)
        test_split = dask.delayed(csr_matrix)(URM_test,
                                              shape=(num_users, num_items),
                                              dtype=np.int32)
        ...
        if not isspmatrix_csr(train_split) or not isspmatrix_csr(validation_split) or not isspmatrix_csr(
                test_split):
            logger_message = ("Reshaping splits - "
