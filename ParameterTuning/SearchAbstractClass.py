...
class SearchAbstractClass(object):
    ...
    def _fit_model(self, current_fit_parameters):
        ...
        log_message = f"{self.ALGORITHM_NAME}: Testing config: {current_fit_parameters}"
    ...
    def _evaluate_on_test(self, recommender_instance, current_fit_parameters_dict, print_log = True):
        ...
        if print_log:
            log_message = f"{self.ALGORITHM_NAME}: Best config evaluated with evaluator_test. Config: {current_fit_parameters_dict} - results:\n{result_string}\n"
    ...
    def _evaluate_on_test_with_data_last(self):
        ...
        log_message = (f"{self.ALGORITHM_NAME}: Evaluation with constructor data for final test. "
                       f"Using best config: {self.metadata_dict['hyperparameters_best']}")
        self._print(log_message)
        logger.info(log_message)
        ...
        log_message = (f"{self.ALGORITHM_NAME}: Best config evaluated with evaluator_test with constructor data for final test. "
                       f"Config: {self.metadata_dict['hyperparameters_best']} - "
                       f"results:\n{result_string}\n")
        self._write_log(log_message)
        logger.info(log_message)
        ...
        if self.save_model in ["all", "best", "last"]:
            log_message = f"{self.ALGORITHM_NAME}: Saving model in {self.output_folder_path + self.output_file_name_root}"
            self._print(log_message)
            logger.info(log_message)
    ...
    def _objective_function(self, current_fit_parameters_dict):
        ...
        try:
            ...
            if new_best_config_found:
                log_message = (f"{self.ALGORITHM_NAME}: New best config found. Config {self.model_counter}: "
                               f"{current_fit_parameters_dict} - "
                               f"results: {result_string}")
                self._write_log(log_message)
                logger.info(log_message)
                ...
                log_message = (f"{self.ALGORITHM_NAME}: "
                               f"Config {self.model_counter} is suboptimal. "
                               f"Config: {current_fit_parameters_dict} - "
                               f"results: {result_string}")
                self._write_log(log_message)
                logger.info(log_message)
