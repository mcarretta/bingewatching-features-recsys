...
class SearchBayesianSkopt(SearchAbstractClass):
    ...
    def _resume_from_saved(self):
        ...
        log_message = (f"{self.ALGORITHM_NAME}: "
                       f"Resuming '{self.output_file_name_root}'... Loaded {self.model_counter} configurations.")
    ...
    def search(self, recommender_input_args,
        ...
        logger.debug(f"Hyperparameter search random state is {random_state}")

        self._set_skopt_params(random_state=random_state)  # default parameters are set here
        ...
        if self.n_loaded_counter < self.model_counter:
            log_message = (f"{self.ALGORITHM_NAME}: Search complete. "
                           f"Best config is {self.metadata_dict['hyperparameters_best_index']}: {self.metadata_dict['hyperparameters_best']}\n")
