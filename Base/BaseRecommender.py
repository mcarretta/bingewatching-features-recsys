...
class BaseRecommender(object):
    ...
    def __init__(self, URM_train, verbose=True, evaluation_block_size: int = 1):
        ...
        # TODO: This should be changed to be dependant on the dataset and the algorithm, not just the latter.
        self.evaluation_block_size = evaluation_block_size

        self._cold_user_mask = np.ediff1d(self.URM_train.indptr) == 0

        if self._cold_user_mask.any():
            logger.info(f"URM Detected {self._cold_user_mask.sum()} "
                        f"({self._cold_user_mask.sum() / self.n_users * 100:.2f} %) cold users.")

        self._cold_item_mask = np.ediff1d(self.URM_train.tocsc().indptr) == 0

        if self._cold_item_mask.any():
            logger.info(f"URM Detected {self._cold_item_mask.sum()} "
                        f"({self._cold_item_mask.sum() / self.n_items * 100:.2f} %) cold items.")
