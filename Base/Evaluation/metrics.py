...
class MAP(_Metrics_Object):
    ...
    def merge_with_other(self, other_metric_object: MAP):
        if not isinstance(other_metric_object, MAP):
            raise ValueError("MAP: attempting to merge with a metric object of different type")
...
class MRR(_Metrics_Object):
    ...
    def merge_with_other(self, other_metric_object: MRR):
        if not isinstance(other_metric_object, MRR):
            raise ValueError("MRR: attempting to merge with a metric object of different type")
...
def ndcg(ranked_list, pos_items, relevance=None, at=None):
    ...
    # if ndcg_ == np.inf:
    #     print(rank_dcg, rank_scores, ideal_dcg)
    return ndcg_


def dcg(scores):
    return np.sum(np.divide(np.power(2, scores) - 1, np.log(np.arange(scores.shape[0], dtype=np.float64) + 2)),
                  dtype=np.float32)
...
class RMSE(_Metrics_Object):
    ...
    def merge_with_other(self, other_metric_object: RMSE):
        if not isinstance(other_metric_object, RMSE):
            raise ValueError("RMSE: attempting to merge with a metric object of different type")
...
class Coverage_User(_Metrics_Object):
    ...
    def merge_with_other(self, other_metric_object: Coverage_User):
        if not isinstance(other_metric_object, Coverage_User):
            raise ValueError("Coverage_User: attempting to merge with a metric object of different type")
...
class Coverage_User_Correct(_Metrics_Object):
    ...
    def merge_with_other(self, other_metric_object):
        if not isinstance(other_metric_object, Coverage_User):
            raise ValueError("Coverage_User_Correct: attempting to merge with a metric object of different type")
...
class Novelty(_Metrics_Object):
    ...
    def merge_with_other(self, other_metric_object: Novelty):
        if not isinstance(other_metric_object, Novelty):
            raise ValueError("Novelty: attempting to merge with a metric object of different type")

        self.novelty += other_metric_object.novelty
...
class AveragePopularity(_Metrics_Object):
    ...
    def merge_with_other(self, other_metric_object: AveragePopularity):
        if not isinstance(other_metric_object, AveragePopularity):
            raise ValueError("AveragePopularity: attempting to merge with a metric object of different type")

        self.cumulative_popularity += other_metric_object.cumulative_popularity
        self.n_evaluated_users += other_metric_object.n_evaluated_users
...
class Diversity_similarity(_Metrics_Object):
    ...
    def merge_with_other(self, other_metric_object):
        if not isinstance(other_metric_object, Diversity_similarity):
            raise ValueError("Diversity: attempting to merge with a metric object of different type")
...
class Diversity_MeanInterList(_Metrics_Object):
    ...
    def merge_with_other(self, other_metric_object: Diversity_MeanInterList):
        if not isinstance(other_metric_object, Diversity_MeanInterList):
            raise ValueError("Diversity_MeanInterList: attempting to merge with a metric object of different type")
